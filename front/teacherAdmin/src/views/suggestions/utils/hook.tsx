...
import { getSuggestionsList, deleteSuggestions } from "@/api/suggestions";
...
export function useSuggestions(tableRef: Ref) {
  ...
  async function handleDelete(row) {
    const { success } = await deleteSuggestions({ ids: [row.id] });
    if (success) {
  ...
  async function onbatchDel() {
    ...
    const { success } = await deleteSuggestions({
  ...
  async function onSearch() {
    loading.value = true;
    const { success, data } = await getSuggestionsList({
      search: toRaw(form),
