...
import { getShortSuggestionList, addShortSuggestion, addVote } from '@/api/schSuggestion';

export function useSchSuggestionShort() {
  ...
  const fetchItems = async () => {
    ...
    const res = await getShortSuggestionList({
      page: page.value,
      pageSize,
  ...
  const vote = async () => {
    ...
    const res = await addVote({ id: curId.value, type: 3 });
    const { success, data } = res.data;
    if (success) {
  ...
  const handleAddSubmit = async (form: {
    ...
    const res = await addShortSuggestion({ ...form });
    const { success, data } = res.data;
    if (success) {
